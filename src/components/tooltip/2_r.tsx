...
const Tooltip = ({id, title, description}: {
    ...
    return (
        <div className={cx('container')}>
            <button className={cx('trigger')} onClick={handleClick}>{title}</button>
            {isOpen && <div className={cx('tooltip')} onClick={e => e.stopPropagation()}>{description}</div>}
        </div>
    );
}
...
const Tooltip2 = () => {
    return (
        <>
            <h3>#2. React<sub>하나의 툴팁만 열리는 context 도입</sub></h3>
